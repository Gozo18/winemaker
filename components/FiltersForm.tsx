...
export default function FiltersForm({ id }: any) {
  const { email, setAddFiltersVisibility, setWinesLoading } = useStateContext()
  ...
  const submitNote = async () => {
    try {
      await addDoc(
        collection(db, "winemakers", email, "wines", id, "filters"),
        {
          date: String(date),
          note: note,
        }
      )
      setAddFiltersVisibility(false)
      setWinesLoading(false)
      toast.success("Filtrace přidána!")
    } catch (err) {
      console.log(err)
